...
class Compile extends Component {
  ...
  componentDidMount() {
	...
	if(oldcode==null){
		fetch(ProblemSet.leetcodes[id][0]).then(res => res.text()).then(text => {
	...
	fetch(ProblemSet.statements[id]).then(res => res.text()).then(text => {
		this.setState({ description: text})
	});
	  
	fetch(ProblemSet.Test[id]).then(res => res.text()).then(text => {
		this.setState({ test: text})
	});
	  fetch(ProblemSet.Submit[id]).then(res => res.text()).then(text => {
